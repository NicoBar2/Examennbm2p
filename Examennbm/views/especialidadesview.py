...
class EspecialidadState(rx.State):
    ...
    @rx.background
    async def get_todas_especialidades(self):
        async with self:
            self.especialidades = servicio_especialidades_all()

    @rx.background
    async def get_especialidad_nombre(self):
        async with self:
            self.especialidades = servicio_consultar_especialidad(
                self.buscar_nombre)
    ...
    @rx.background
    async def crear_especialidad(self, data: dict):
        async with self:
            try:
...
def especialidad_page() -> rx.Component:
    return rx.flex(
        rx.heading("Especialidades", title="Especialidades",
                   size="5", center=True),
        rx.vstack(
...
def buscar_especialidad_nombre() -> rx.Component:
    return rx.hstack(
        rx.input(placeholder="Nombre",
                 on_change=EspecialidadState.buscar_onchange),
        rx.button("Buscar especialidad",
                  on_click=EspecialidadState.get_especialidad_nombre)
    )
